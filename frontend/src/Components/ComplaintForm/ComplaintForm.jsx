...
function ComplaintForm() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setSuccessMessage(t('complaint.submitted_successfully'));
        } catch (err) {
            console.error('Error submitting complaint:', err);
        } finally {
            setIsSubmitting(false);
                    ...
                    <div className="card shadow-lg border-0 rounded-3" style={{ background: '#f0f7ff' }}>
                        <div className="card-body p-5">
                            ...
                            <form onSubmit={handleSubmit}>
                                ...
                                <div className="mb-4">
                                    <label htmlFor="description" className="form-label">{t('complaint.describe_issue')}</label>
                                    <textarea
                                        id="description"
                                        className="form-control"
                                        rows="5"
                                        placeholder={t('complaint.provide_details')}
                                        style={{ borderRadius: '10px', borderColor: '#71b0f4' }}
                                        value={description}
                                        onChange={(e) => setDescription(e.target.value)}
                                    ></textarea>
